...
# Modelar sistema bancário em POO

# Adicionar classes para cliente e operações bancárias depósito e saque

# Representação em UML
# c = classe, i = interface

# Conta(c) tem um Historico(c), Conta pode ser uma ContaCorrente(c)
# Cliente(c) vai ter várias Contas, Cliente é PessoaFisica(c)
# Cliente realiza Transacao(i), Deposito ou Saque, e Historico armazena transacoes

# Conta possui os atributos saldo, numero, agencia, cliente, historico e métodos saldo(), nova_conta(), sacar() e depositar()
# ContaCorrente possui atributos limite e limite_saques
# Cliente possui atributos endereco, contas e métodos realizar_transacao(), adicionar_conta()
# PessoaFisica possui atributos cpf, nome e data_nascimento
# Transacao possui metodo registrar
# Deposito e Saque são atributos de Transacao
# Historico possui método adicionar_transacao()
...
def extrato(clientes):
    ...
    if cliente:
        ...
        conta = retornar_conta(nro_conta, cliente.contas)
        if conta:
            historico = conta.historico.retornar_historico()

            if len(historico) != 0:
                # Se historico não estiver vazio, imprime ele na tela
                print("="*50)
                for cont, lista in enumerate(historico):
