...
# Implemente ao sistema bancário um decorador de log, adicionando às transações a função de printar a data e hora, como também o tipo da transação.
# Crie um gerador para iterar sobre as transações de contas e retorne uma a uma, com uma opção de filtrar por tipo
# Adicione um iterador personalizado, que faça a iteração sobre as contas do banco, retornando informações básicas de cada conta
...
class Historico:
    ...
    def gerador_historico(self, tipo=None):
        ...
        # Código antigo, mantido por fins didáticos
        ''' 
        if tipo == "Deposito":
            for transacao in self._historico:
